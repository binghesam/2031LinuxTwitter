# in this file changes, I will add with and kill processing part to optimize the running of the python code
# to avoid the problem of running out of the ram in pc
...
configList=[config0,config1,config2,config3,config4,config5]
configListB=[config0,config1b,config2b,config3b,config4,config5]
...
all1=[-180,-90,180,90]
all2=[-180,-90,180,90]
locationList=[Seattle,La,Ny,Chicago,all1,all2]
locationListName=["Seattle","la","ny","ch","all01","all02"]
...
class StdOutListener(StreamListener):
    ...
    def on_data(self, data):
    	#in the later testing, i found that data is just one tweet for processing.
...
if __name__ == '__main__':
    ...
    loopCounter=0
    mainWhileLoopFlag=True
    while mainWhileLoopFlag:
        #This handles Twitter authetification and the connection to Twitter Streaming API
        #print("config showing\n")
        
        #by bing newly added for the debugging
        print("start a new while loop")

        if loopCounter>=1:
            stream.disconnect()
            del stream
            del listener # by bing this is my added files
            del auth # by bing, add new more things to test it.
            print("use del to delete the stream, listener, auth")
            print("the current loopCounter is: {0}".format(loopCounter))

            #by bing for the os debugging
            # loopCounter is set for the whole loop time with loopCounter*timePeriod(3mins OR 1hour condition)
            if loopCounter==1:
                mainWhileLoopFlag=False
                print("loop Counter limit reached, then while loop will stop later")
        ################by testing
        ...
        stream = Stream(auth, listener, timeout=60, async=True)
        ...
        try:
            ...
            #by bing, real running with the time
            stream.filter(locations=locSpec)

            # #by bing,used when testing the thread debugging.
